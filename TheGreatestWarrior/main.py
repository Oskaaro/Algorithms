...
class Warrior:
    def __init__(self):
        self.level = 1
        self.rank = Rank(self.level // 10).name
        self.experience = 100
        self.achievements = []

    def update_level_rank(self, exp):
        self.experience += exp
    ...
    def training(self, achievement):
        if achievement[2] > self.level:
            return "Not strong enough"
        self.achievements.append(achievement[0])
        self.update_level_rank(achievement[1])
        return achievement[0]

    def battle(self, enemy):
        if enemy <= 0 or enemy > 100:
            return "Invalid level"
        level_difference = enemy - self.level
        if level_difference >= 5 and self.level // 10 < enemy // 10:
            return "You've been defeated"
        if level_difference <= -2:
            return "Easy fight"
        match level_difference:
            case 0:
                self.update_level_rank(10)
                return "A good fight"
            case -1:
                self.update_level_rank(5)
                return "A good fight"
            case default:
                self.update_level_rank((level_difference ** 2) * 20)
